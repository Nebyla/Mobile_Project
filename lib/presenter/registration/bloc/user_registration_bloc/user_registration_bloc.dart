...
class UserRegistrationBloc
  ...
  UserRegistrationBloc({required this.onRegistered})
  ...
  _fistnameChanged(
      FirstNameChanged event, Emitter<UserRegistrationState> emit) {
  ...
  _middlenameChanged(
      MiddleNameChanged event, Emitter<UserRegistrationState> emit) {
    emit(state.copyWith(middleName: event.middleName));
  }

  _firstPasswordChanged(
      FirstPasswordChanged event, Emitter<UserRegistrationState> emit) {
    emit(state.copyWith(firstPassword: event.firstPassword));
  }

  _secondPasswordChanged(
      SecondPasswordChanged event, Emitter<UserRegistrationState> emit) {
    emit(state.copyWith(secondPassword: event.secondPassword));
  }

  _onRegistration(RegistrationButtonPressedEvent event,
